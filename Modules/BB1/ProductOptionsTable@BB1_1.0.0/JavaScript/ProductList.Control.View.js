...
define('ProductList.Control.View', [
	...
	, 'ProductList.ControlItem.View', 'ProductList.ControlNewItem.View', 'Profile.Model', 'ProductList.Model', 'ProductList.Item.Model', 'Session', 'ProductList.Item.Collection', 'MenuTree.View'
	...
	, ControlItemView, ControlNewItemView, ProfileModel, ProductListModel, ProductListItemModel, Session, ProductListItemCollection, MenuTreeView
	...
	return Backbone.View.extend({
		...
		addItemToList: function(line, productList, dontShowMessage) {
				...
				} else //Multi add
				{
					...
					var matrixquantities = line.get("matrixquantities");

					if (!matrixquantities) {
						return;
					}
					var optionlist = [];
					var item_matrix_children = line.get('item').get('_matrixChilds');
					var options = line.getVisibleOptions();
					var newOption;
					...
					var lineoptions, lines = [];
					var cartOptionId, itemOptionId, values, label;
					//first set the options on the model
					for (var i = 0; i < optionlist.length; i++) {

						lineoptions = line.get("options");
				...
				destination.get('items').create(original_item_clone, {
					...
					success: function(saved_model) {
						...
						app.getLayout().currentView.showConfirmationMessage(
							_('<div class="product-list-control-message">Good! You successfully moved the item from this to <a href="/wishlist/$(0)">$(1)</a></div>').translate(destination.get('internalid'), destination.get('name'))
						);
					}
				});
			}
		...
		showFlyout: function(e) {
				...
				var matrixquantities = this.product.get("matrixquantities") || {};

				e.stopPropagation();
				e.preventDefault();
				var valid = false;
				for (var q in matrixquantities) {

					if (matrixquantities[q] > 0) {
						valid = true; //there must be a valid item.
					}
				}

				//Here no extra validation is specified as those are not required to add an item into a product list
				if (valid||this.product.get('item').get('_matrixChilds').length==0) {
					...
					SC.Tools.showErrorInModal(this.application, _.translate("Add to My Tool List"), _.translate("Please select a quantity for at least one option. Then click 'Add to My Tool List'."));
				}
			}
				...
				if (e.which !== 2 && e.which !== 3) {
					...
					_.delay(function() {
						if (!self.countedClicks[e.pageX + '|' + e.pageY]) {
							if ($(document.elementFromPoint(e.clientX, e.clientY)).hasClass("modal-backdrop")) {
								return;
							}
