...
define('ProductDetailToQuote.View', [
	'SC.Configuration', 'Utils', 'GlobalViews.Message.View', 'ProductList.Model', 'ProductList.Item.Model', 'Profile.Model', 'Session'

	, 'product_detail_to_quote.tpl'

	, 'Backbone.CompositeView', 'Backbone', 'underscore'
], function(
	Configuration, Utils, GlobalViewsMessageView, ProductListModel, ProductListItemModel, ProfileModel, Session

	, productDetailToQuote_tpl

	, BackboneCompositeView, Backbone, _
) {
	'use strict';
	...
	return Backbone.View.extend({
		...
		itemToQuote: function itemToQuote(e) {
				...
				if (this.profile_model.get('isLoggedIn') === 'T' && !this.state.quote_permissions) {
					console.log("itemToQuote permission");
					...
					SC.Tools.showErrorInModal(this.application, _("Add to Quote").translate(), this.state.feedbackMessage);
					this.render();
				} else {
					...
					if ((valid || this.model.isSelectionComplete()) && this.isQuotable() && this.validateLogin()) {
						...
						if (!this.isQuotable()) {
							SC.Tools.showErrorInModal(this.application, _.translate("Add to Quote"), _.translate("Unable to add this item to a quote."));
						}
						if (!this.validateLogin()) {
							//SC.Tools.showErrorInModal(this.application, _.translate("Add to Quote"), _.translate("Please login to create quotes."));
						} else {
							SC.Tools.showErrorInModal(this.application, _.translate("Add to Quote"), _.translate("Please select a quantity for at least one option. Then click 'Add to Quote'."));
						}
					}
				}
			}
		...
		addItemToQuote: function(product_list, product_list_line, product) {
				...
				product_list_line.save(null, {
					...
					if (show_enforced_quantity_message) {
						...
						SC.Tools.showErrorInModal(self.application, _("Add to Quote").translate(), self.state.feedbackMessage);
