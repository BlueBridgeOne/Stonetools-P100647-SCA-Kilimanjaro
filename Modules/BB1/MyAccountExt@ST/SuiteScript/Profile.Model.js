	...
	)
{
	...
	,	get: function ()
		{
			//nlapiLogExecution("DEBUG","Get Profile","online="+ModelsInit.session.isLoggedIn2());
			var profile = {};
			...
			if (ModelsInit.session.isLoggedIn2() && this.isSecure)
			{
				...
				this.fields = this.fields || ['isperson', 'email', 'internalid', 'name', 'overduebalance', 'phoneinfo', 'companyname', 'firstname', 'lastname', 'middlename', 'emailsubscribe', 'campaignsubscriptions', 'paymentterms', 'creditlimit', 'balance', 'creditholdoverride','vatregistration'];

				
				profile = ModelsInit.customer.getFieldValues(this.fields);

				profile.vatregnumber=profile.vatregistration;
				profile.vatregistration=undefined;
			...
			//nlapiLogExecution("DEBUG","Get Profile","Profile="+JSON.stringify(profile));
			return profile;
		}
	...
	,	update: function (data)
		{
			...
			customerUpdate.vatregistration=data.vatregnumber;
