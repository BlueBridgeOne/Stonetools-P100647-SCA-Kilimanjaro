...
{{#if isProductList}}
			...
			<div class="list-header-view-share-header">
				<a class="list-header-view-csv" data-action="download-csv" title="{{translate 'Download CSV'}}">
					 <i class="list-header-view-csv-icon" ></i>
				</a>
				<a class="list-header-view-print" data-navigation="ignore-click" onclick="window.print();"><i class="list-header-view-print-icon"></i> </a>
<a class="list-header-view-email" target="_blank" data-navigation="ignore-click" data-action="send-email"><i class="list-header-view-email-icon"></i> </a>
				<span class="list-header-view-share-text">Share</span>
			</div>

{{#if sends}}
<div class="list-header-view-send">
	
		<table><tr><td class="no-padding">
{{translate 'Email this list to me'}}:
</td><td class="no-padding">
<select name="auto-send" class="list-header-view-accordion-body-select list-header-send-select" data-action="auto-send">
									{{#each sends}}
										<option value="{{value}}" {{#if selected}} selected {{/if}}>{{translate name}}</option>
									{{/each}}
								</select>
</td></tr></table>
</div>
