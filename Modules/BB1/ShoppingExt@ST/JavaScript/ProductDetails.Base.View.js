	...
	)
{
	...
	var ProductDetailsBaseView = Backbone.View.extend(
		...
		,	childViews: {
				...
				,
				'Product.OptionsTable': function ()
				{
					return new ProductOptionsTableView({
						model: this.model
					,	application: this.application
					,	show_pusher: this.showOptionsPusher()
					,	show_required_label: this.model.get('item').get('itemtype') === 'GiftCert'
		...
		,	getContext: function ()
			{
				...
				var item_matrix_children = item_model.get('_matrixChilds');
				...
				,isMatrix:item_matrix_children.length>0
