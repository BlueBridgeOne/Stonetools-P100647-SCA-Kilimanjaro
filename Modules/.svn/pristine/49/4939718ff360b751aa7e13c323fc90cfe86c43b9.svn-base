...
define('ListHeader.View', [
	'SC.Configuration', 'GlobalViews.Pagination.View', 'GlobalViews.ShowingCurrent.View', 'AjaxRequestsKiller', 'ToolLists.Email.View', 'ToolLists.Model', 'list_header_view.tpl'

	, 'Backbone', 'Backbone.CompositeView', 'underscore', 'jQuery', 'Utils', 'bootstrap-datepicker', 'UrlHelper'
], function(
	Configuration, GlobalViewsPaginationView, GlobalViewsShowingCurrentView, AjaxRequestsKiller, ToolListsEmailView, ToolListsModel, list_header_view_tpl
	...
	var ListHeader = Backbone.View.extend({
		...
		events: {
			...
			'change [data-action="auto-send"]': 'autoSend'
		...
		autoSend: function(e) { //Auto send this list very....

			var description = this.collection.description; //set the send option on the description. Defaults to never.
			var newsend = e.target.value;
			var values = description.split(',');
			var send = "never";
			var newdesc = "";
			var found=false,foundlast=false;
			...
			date=date.substring(0,T);
				...
				newdesc += ",lastsend:"+date;
			}
			this.collection.description=newdesc;
			console.log(newdesc);
			this.options.view.model.set('description',newdesc);
			this.options.view.model.save();
			
		},
		downloadCSV: function(e) {
			console.log("Download CSV " + this.productListId);


			document.location = _.getAbsoluteUrl('services/ToolLists.Service.ss?task=download-csv&productListId=' + this.productListId);

		},
		sendEmailCallback: function(e) { //callback function, send the email.
			console.log("email " + e.email_value);
			var emailmodel = new ToolListsModel();
			var self = e.context;
			emailmodel.fetch({ data: { email: e.email_value, task: "email-csv", productListId: self.productListId } }).done(function(res) {
				console.log("done.");
			}).error(function(res) {
				console.log(self.options.application);
				console.log(res.responseJSON.errorMessage);
				SC.Tools.showErrorInModal(self.options.application, _("Unable to Send Email").translate(), res.responseJSON.errorMessage);
			});

		},
		sendEmail: function(e) { //open the email dialog
				console.log("Send Email " + this.productListId);

				e.preventDefault();

				var sendEmailView = new ToolListsEmailView({
					callBack: this.sendEmailCallback,
					callBackParameters: {
						context: this
					}
				});

				this.options.application.getLayout().showInModal(sendEmailView);
		...
		initialize: function(options) {
			...
			this.productListId = options.collection.productListId;
		...
		getContext: function() {
				...
				var sends;
				if (isProductList) {
					var description = this.collection.description; //get the send option from the description. Defaults to never.
					var values = description.split(',');
					var send = "never";
					...
					sends = [
						{ name: "Never", value: "never", selected: send == "never" },
						{ name: "Every Week", value: "week", selected: send == "week" },
						{ name: "Every Two Weeks", value: "twoweeks", selected: send == "twoweeks" },
						{ name: "Every Month", value: "month", selected: send == "month" },
						{ name: "Every Two Months", value: "twomonths", selected: send == "twomonths" },
						{ name: "Every Three Months", value: "threemonths", selected: send == "threemonths" },
						{ name: "Every Six Months", value: "sixmonths", selected: send == "sixmonths" },
						{ name: "Every Year", value: "year", selected: send == "year" }
					];
