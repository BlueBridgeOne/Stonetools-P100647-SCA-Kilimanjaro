...
define('Facets.Translator', ['underscore', 'jQuery', 'SC.Configuration', 'Utils', 'UrlHelper'], function(
    _, jQuery, Configuration
) {
    'use strict';
...
query=path.substring(Q);
            ...
            var categoryUrl = fragments.slice(0, index).join("/");

            facets = fragments.slice(index).join("/")+query;
    ...
    _.extend(FacetsTranslator.prototype, {
        ...
        getCategoryUrl: function getCategoryUrl() {
                ...
                var Q = this.categoryUrl.indexOf("?");
                if (Q > -1) {
                    return this.categoryUrl.substring(0, Q);
                }
        ...
        getApiParams: function getApiParams() {
                ...
                params.custitem_cseg_bb1_website_se = "Any," + SC.Tools.getSiteCountry();

                if (this.isCategoryPage) {
                    params.commercecategoryurl = this.categoryUrl;
                    var Q = params.commercecategoryurl.indexOf("page=");
                    if (Q > -1) {
                        params.offset = 48 * (parseInt(params.commercecategoryurl.substring(Q + 5)) - 1);
                    }
                    Q = params.commercecategoryurl.indexOf("?");
                    if (Q > -1) {
                        params.commercecategoryurl = params.commercecategoryurl.substring(0, Q);
