	...
	)
{
	...
	,	showContent: function ()
		{
			...
			,	resultCount = this.model.get('total')||0;
	...
	,	childViews: {
			'Facets.FacetedNavigation': function (options)
			{
				...
				,	has_items = this.model.get('items')&&this.model.get('items').length
	...
	,	getContext: function()
		{
			...
			isCategories:hasSubcategories>0,
			listClass:hasSubcategories>0?"w-100":"facets-facet-browse-results"
			,isSearch:isSearch
			};
