...
define(
	'Account.Model', [
		'SC.Model', 'Application', 'SC.Models.Init', 'Profile.Model', 'LiveOrder.Model', 'Address.Model', 'CreditCard.Model', 'SiteSettings.Model', 'underscore'
	],
	function (
		SCModel, Application, ModelsInit, Profile, LiveOrder, Address, CreditCard, SiteSettings, _
	) {
		'use strict';
		...
		return SCModel.extend({
			...
			register: function (user_data) {
				...
				if (ModelsInit.customer.isGuest()) {
					...
					var res = ModelsInit.session.registerCustomer({
						firstname: user_data.firstname,
						lastname: user_data.lastname,
						companyname: user_data.company,
						email: user_data.email,
					...
					try {
						//nlapiLogExecution("ERROR", "Register PW", JSON.stringify(res)+" "+user_data.password);
						var customerUpdate = {
							internalid: res.internalid,
							vatregnumber:user_data.vatregnumber,
							vatregistration:user_data.vatregnumber,
							customfields: {
								custentity_bb1_sca_initialcustpassword: user_data.password
							}
						};
						ModelsInit.customer.updateProfile(customerUpdate);

					} catch (err) {
						nlapiLogExecution("ERROR", "Register", err);
					}
