...
<div class="cart-confirmation-modal">
	...
	{{#each lines}}
	<p><span class="cart-confirmation-modal-sku">{{{optiontext}}}</span></p>
	<p><span class="cart-confirmation-modal-price">{{rate_formatted}}</span>&nbsp;<span class="price-ex-vat">{{translate 'Ex. VAT'}}</span>
	{{#if ../showQuantity}}
		
			&nbsp;&nbsp;&nbsp;&nbsp;
				<span class="cart-confirmation-modal-quantity-label">{{translate 'Quantity: '}}</span>
				<span class="cart-confirmation-modal-quantity-value">{{quantity}}</span>
		{{/if}}
</p>
<br />
	{{/each}}
		...
		<div class="cart-confirmation-modal-actions">
			<div class="cart-confirmation-modal-view-cart">
				<a href="/cart" class="cart-confirmation-modal-view-cart-button">{{translate 'View Basket &amp; Checkout'}}</a>
			</div>
